...
const double HACK_CAM_CAL_X_OFFSET = 0.0165;
...
void ImageConverter::imageCb(const sensor_msgs::ImageConstPtr& msg){
	...
	cam_coords_xy = cam_coords_ij/pix_per_m;
        ...
        //wsn added the next 3 lines, 12/12/18
        Eigen::Vector2d major_axis_vec_xy;
        major_axis_vec_xy= g_R_camera*major_axis_vec;
        double theta=atan2(major_axis_vec_xy(1),major_axis_vec_xy(0));
        
        //hack for cam-cal manual correction:
        block_pose_.pose.position.x = xy_robot_derived(0) + HACK_CAM_CAL_X_OFFSET; 
        block_pose_.pose.position.y = xy_robot_derived(1); 
